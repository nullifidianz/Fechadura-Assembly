...
clearDisplay :
        ...
        SETB E ; |
        CLR E ; | negative edge on E
        CALL Atraso ; wait for BF to clear
        RET
...
;-----------------------------------------------------------------------------------------
;--------------------------------Alterar senha------------------------------------------------
IterarSenhaNova: Call VarreduraSenhaNova	; Varre a entrada do teclado
        CALL clearDisplay
        MOV A, #00h
...
;------------------- Verifica o número de códigos corretos inseridos ---------------		
		Clr A
		Inc R4
		Cjne R4,#04h,IterarNovaSenhaCompleta
...
VarreduraSenhaNova:	
			;Varre Linha0			
			CLR P0.0			;Limpa Linha0
			CALL IDCodigo3SenhaNova		;Chama sub-rotina de varredura de coluna
			SetB P0.0			;Define Linha 0
			JB F0,Feito2			;Se F0 estiver definido, encerra a varredura 
														
			JMP VarreduraSenhaNova	;Volta para varrer Linha3
Feito2:		Clr F0		        ;Limpa a bandeira F0 antes de sair
			Ret

IDCodigo3SenhaNova:	JNB P0.4, CodigoChave00SenhaNova	;Se Col0 Linha0 estiver limpa - chave encontrada
			RET			

CodigoChave00SenhaNova:	SETB F0			;Chave encontrada - define F0
			Mov R7,#'#'		;Código para '#' 
			RET				
;--------------------------------------------------------------------------------			
;-----------------------------------PosicaoCursor------------------------------------------
