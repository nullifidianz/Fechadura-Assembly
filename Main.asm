...
RS	Equ	P1.3
E	Equ	P1.2
; Defina o registrador P2 para entrada
MOV P2, #0xFF ; Defina todos os pinos de P2 como entrada (todos os bits são 1)

; ---------------------------------- Main -------------------------------------
Main:
		Clr RS		; RS=0 - Registro de instrucoes esta selecionado.

		Call ConfigFunc	; Configuracao da funcao
		...
		Mov DPTR,#LUT1	; Tabela de consulta para a mensagem "Digite o PIN:" 
            MOV A, P2 ; Move o estado do P2 para o acumulador (A)
          
            ; Verifique se o botão está pressionado
            ANL A, #00000001B ; Verifica o bit menos significativo (P2.0)
            JNZ ButtonPressed ; Se o bit estiver definido, vá para ButtonPressed
      
            ; Se o botão não estiver pressionado, faça alguma coisa aqui
            ; Por exemplo, piscar um LED
            SJMP MAIN ; Voltar para o loop principal

ButtonPressed:
          ; Se o botão estiver pressionado
          SJMP MAIN ; Voltaar para o loop principal
Novamente:	Clr A
		Movc A,@A+DPTR	; Obtem o caractere
		Jz Proximo		; Sai quando A=0
		Call EnviarCar	; Exibe o caractere
		Inc DPTR		; Aponta para o proximo caractere
		Jmp Novamente
	
Proximo: Mov R4,#00h	; Contador para verificar o numero de atualizacoes
		Mov R5,#00h	; Contador para verificar o numero de entradas corretas
		Mov DPTR,#LUT4	; Copia o inicio da tabela de consulta para o PIN
;----------------------------------- Obter Entrada ----------------------------------		
...
;------------------- Verifica PINS corretos inseridos ---------------		
		...
		Call VerificarEntrada	; Verifica o numero de entradas corretas
		Inc DPTR
		Inc R4
		Cjne R4,#04h,Iterar
		
		Cjne R5,#04h,Incorreto	; Verifica o numero de entradas corretas
Correto: Call PosicaoCursor  	; Coloca o cursor na proxima linha
		SetB RS		; RS=1 - Registro de dados esta selecionado.
		Call Concedido
		; Ativa o motor no sentido anti-horario
		SETB P3.0 ; Supondo que P3.0 seja o pino de controle para girar o motor
		CLR P3.1 ; Se necessario, dependendo do seu circuito

		; Inicia temporizador para uma volta e meia
		MOV R6, #0 ; Contador de temporizacao
...
Temporizar:	DJNZ R7, Temporizar ; Aguarda completar uma volta e meia
		...
		Jmp FimAqui
Incorreto: Call PosicaoCursor  	; Coloca o cursor na proxima linha
		SetB RS		; RS=1 - Registro de dados esta selecionado.
		Call Negado
FimAqui: Jmp $
;-------------------------------- Sub-rotinas ---------------------------------				
ConfigFunc:	Clr  P1.7		
		Clr  P1.6		
		SetB P1.5		
		Clr  P1.4		; | (DB4)DL=0 - coloca o modulo LCD em modo de 4 bits 
